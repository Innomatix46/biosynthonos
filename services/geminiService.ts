...
import { SimulationResult, PedProtocol, ProtocolPhase, AthleteProfile, TranslatableText } from '../shared/types';
 ...
 */
const constructAnalysisPrompt = (result: SimulationResult): string => {
  ...
  const summaryData = {
    ...
    deterministicSummary: JSON.stringify(result.summary),
    deterministicWarnings: JSON.stringify(result.warnings),
  };
...
export const getAiAnalysis = async (result: SimulationResult, t: TFunction): Promise<TranslatableText> => {
  try {
    const prompt = constructAnalysisPrompt(result);
...
const constructSuggestionPrompt = (profile: AthleteProfile): string => {
    ...
    return `
        ...
        4.  **Dosages**: Use conservative, commonly cited dosages. Prioritize safety over extreme results.
...
export const getAiProtocolSuggestion = async (profile: AthleteProfile, t: TFunction) => {
    try {
        const prompt = constructSuggestionPrompt(profile);
