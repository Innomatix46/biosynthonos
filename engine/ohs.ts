...
const calculateNextWeekBloodMarkers = (profile: AthleteProfile, hpsResult: HpsResult, previousMarkers: BloodMarker[]): BloodMarker[] => {
    ...
    markers.push({ marker: 'eGFR', value: newGfr.toFixed(0), status: newGfr < 30 ? 'critical' : newGfr < 60 ? 'low' : 'normal', notes: `Filtration rate affected by renal strain score of ${gfrStrain.toFixed(1)}.` });
    
    // --- Cardiovascular Model ---
    const sysBP = getPrev('Systolic BP', 120);
    const newSysBP = sysBP + totalCardioToxicity * 0.4 - totalBloodPressureReduction * 0.5;
    markers.push({ marker: 'Systolic BP', value: newSysBP.toFixed(0), status: newSysBP > 140 ? 'critical' : newSysBP > 130 ? 'elevated' : 'normal', notes: `Influenced by cardio-toxicity and support drugs.` });
    
    const diaBP = getPrev('Diastolic BP', 80);
    const newDiaBP = diaBP + totalCardioToxicity * 0.2 - totalBloodPressureReduction * 0.25;
    markers.push({ marker: 'Diastolic BP', value: newDiaBP.toFixed(0), status: newDiaBP > 90 ? 'critical' : newDiaBP > 85 ? 'elevated' : 'normal', notes: `` });

    let hdl = getPrev('HDL-C', 50);
    hdl -= totalCardioToxicity * 0.2;
    if (geneticFactors.includes('genetics.lipid_response')) hdl -= totalCardioToxicity * 0.1;
    const finalHdl = Math.max(5, hdl); // Enforce a minimum physiological floor
    markers.push({ marker: 'HDL-C', value: finalHdl.toFixed(0), status: finalHdl < 30 ? 'critical' : finalHdl < 40 ? 'low' : 'normal', notes: `Suppressed by cardio-toxicity.` });

    let ldl = getPrev('LDL-C', 100);
    ldl += totalCardioToxicity * 0.3;
    if (geneticFactors.includes('genetics.lipid_response')) ldl += totalCardioToxicity * 0.15;
    markers.push({ marker: 'LDL-C', value: ldl.toFixed(0), status: ldl > 160 ? 'critical' : ldl > 130 ? 'elevated' : 'normal', notes: `Elevated by cardio-toxicity.` });
    ...
    if (geneticFactors.includes('genetics.aromatization')) baseE2 *= 1.05;
    ...
    // --- Other markers ---
    const newGlucose = getPrev('Glucose', 85) + (hpsResult.metabolicAdjustmentFactor - 1) * 10; // GH/Anabolics slightly raise glucose
    ...
    markers.push({ marker: 'Hematocrit', value: hematocrit.toFixed(1), status: hematocrit > 52 ? 'critical' : hematocrit > 50 ? 'elevated' : 'normal', notes: `Increased by androgenic load.` });
...
export const runOhsWeekSimulation = (profile: AthleteProfile, hpsResult: HpsResult, previousMarkers: BloodMarker[]): OhsResult => {
  ...
  const riskScores = {
    cardiovascular: calculateRisk(totalCardioToxicity * 1.5, profile.age, profile.geneticFactors.includes('genetics.cardio_risk'), "Based on lipid impact, androgen load, and genetics."),
