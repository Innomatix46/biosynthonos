              ...
              <li key={report.id}>
                ...
                >
                  <div className="flex items-center justify-between text-sm">
                    <span className="font-medium text-gray-300 flex items-center gap-2 truncate"><FileText size={14}/> {report.fileName}</span>
                    <span className="text-xs text-gray-400 shrink-0 ml-2">{new Date(report.date).toLocaleDateString()}</span>
                  </div>
                </button>
              </li>
            ))}
